...
<div class="row">
    ...
    <div class="box">
        <div class="box-header">
          <h3 class="box-title">List of Hospital Received Blood Stock</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
    ...
    <div class="box">
        <div class="box-header">
          <h3 class="box-title">List of Hospital Collected Blood Stock</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
...
<script>
    ...
    var allHospital = @json($allHospital);
...
async function callBloodAvailable(){
    ...
    for (const Hospital of allHospital) {

        const myStock = await getBloodStocksByOwner(Hospital.user.id);
        ...
        if (recStock[0].length !== 0) {
            ...
            // console.log( recStock, sendStock, abmS,opS);
...
function updateProgressBarWidth(selector, percentage) {
    ...
    try {
        ...
        const bloodStockListOwner = [];

        for (const hospital of allHospital) {
            const bloodStock = await getTransfersByReceiver(hospital.user.id);
            const bloodStockOwner = await getBloodStocksByOwner(hospital.user.id);
...
// console.log(bloodStockListOwner);
        ...
        if (bloodStockListOwner.length === 0 || (bloodStockListOwner[0] && bloodStockListOwner[0].length === 0)) {
            console.log('The array is empty.');
        } else {
            console.log('The array is not empty.');
            // Call display function for owner
            displayBloodTableOwner(bloodStockListOwner);
        }
    } catch (error) {
        ...
        for (let j = 0; j < bloodStockList.length; j++) {
            ...
            for (let prop in bloodStockList[j]) {
                ...
                for (let i = 0; i < bloodStockList[j][prop].length; i++) {
                    ...
                    allHospital.forEach(function (Hospital) {
                        // console.log(bloodStockList[j][1][i],Hospital.user.id);
                        if (bloodStockList[j][1][i] == Hospital.user.id) {
                            const hospitalTd = document.createElement('td');
                            hospitalTd.textContent = Hospital.user.name;
                            tr.appendChild(hospitalTd);
                        }
                    });

                    const dateTd = document.createElement('td');
                    dateTd.textContent = web3.utils.hexToUtf8(bloodStockList[j][3][i]); // Changed from [3] to [2]
                    tr.appendChild(dateTd);

                    const locationTd = document.createElement('td');
                    locationTd.textContent = bloodStockList[j][4][i];
...
function displayBloodTableOwner(bloodStockListOwner) {
    // Check if DataTable is already initialized on the table and delete it
    ...
    const campaignSlotsPromises = [];
    // console.log(bloodStockListOwner);
    if (bloodStockListOwner.length === 0 || bloodStockListOwner[0].length === 0) {

        campaignsListDiv.innerHTML = '<tr><td colspan="4">No campaigns available.</td></tr>';
    } else {

        for (let j = 0; j < bloodStockListOwner.length; j++) {

            console.log(`Hospital ${j} data:`);

            for (let prop in bloodStockListOwner[j]) {
                console.log(`Property ${prop}:`);

                for (let i = 0; i < bloodStockListOwner[j][prop].length; i++) {
                    console.log(`  Element ${i}:`, bloodStockListOwner[j][prop][i]);
                    const tr = document.createElement('tr');

                    // Create a td for each piece of campaign information
                    const campaignIdTd = document.createElement('td');
                    campaignIdTd.textContent = formatDateFromUniqueId(bloodStockListOwner[j][0][i]);
                    tr.appendChild(campaignIdTd);

                    allHospital.forEach(function (Hospital) {
                        console.log(bloodStockListOwner[j][1][i],Hospital.user.id);
                        if (bloodStockListOwner[j][1][i] == Hospital.user.id) {
                            const hospitalTd = document.createElement('td');
                            hospitalTd.textContent = Hospital.user.name;
                            tr.appendChild(hospitalTd);
                        }
                    ...
                    const dateTd = document.createElement('td');
                    dateTd.textContent = web3.utils.hexToUtf8(bloodStockListOwner[j][2][i]); // Changed from [3] to [2]
                    tr.appendChild(dateTd);

                    const locationTd = document.createElement('td');
                    locationTd.textContent = bloodStockListOwner[j][3][i];
                    tr.appendChild(locationTd);
